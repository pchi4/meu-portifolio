              ...
              <Reveal key={idx} delay={idx * 0.1}>
                <div className="group relative overflow-hidden rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 shadow-sm hover:shadow-xl transition-all p-8 md:p-12">
                  <div className="flex flex-col lg:flex-row gap-12 items-center">
                    ...
                    {/* Lado Direito: Mockup (Ajustado) */}
                    <div className="w-full lg:w-[250px] flex justify-center">
                      {proj.tech.includes("React Native") ||
                      proj.tech.includes("Flutter") ? (
                        <div className="scale-90 md:scale-100">
                          <DeviceMockup
                            imageSrc={`/projects/${proj.name.toLowerCase().replace(/ /g, "-")}.png`}
                          />
                        </div>
                      ) : (
                        <div className="w-full aspect-video lg:aspect-square bg-slate-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center border-2 border-dashed border-slate-300 dark:border-slate-700">
                          <span className="text-slate-400 font-mono text-xs text-center p-4">
                            Preview Web Indisponível
                          </span>
                        </div>
                      )}
                    </div>
